...
test.describe("알림 페이지", () => {
  test("헤더·카테고리 필터·알림 목록이 렌더링된다", async ({ context, page, baseURL }) => {
  ...
  test("채팅 카테고리 선택 시 URL에 category=chat 이 반영된다", async ({
    context,
    page,
    baseURL,
  }) => {
  ...
  test("알림이 없을 때 빈 상태 문구가 표시된다", async ({ context, page, baseURL }) => {
    await addRefreshCookie(context, baseURL);
